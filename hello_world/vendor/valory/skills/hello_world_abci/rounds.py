...
class UpdatePrintCountRound(CollectSameUntilThresholdRound, HelloWorldABCIAbstractRound):
    """A round that updates the print count and prints the count to the screen"""

    payload_class = UpdatePrintCountPayload
    synchronized_data_class = SynchronizedData
    done_event = Event.DONE
    no_majority_event = Event.NO_MAJORITY
    #collection_key = get_name(SynchronizedData.participant_to_selection)
    ...
    def end_block(self) -> Optional[Tuple[BaseSynchronizedData, Event]]:
        ...
        if self.threshold_reached:
            print_count = max(
                [cast(UpdatePrintCountPayload, payload).print_count for payload in self.collection.values()]
            )
            synchronized_data = self.synchronized_data.update(
...
class HelloWorldAbciApp(AbciApp[Event]):
    ...
    transition_function: AbciAppTransitionFunction = {
        ...
        PrintMessageRound: {
            Event.DONE: UpdatePrintCountRound,
            Event.ROUND_TIMEOUT: RegistrationRound,
        },
        UpdatePrintCountRound: {
